function imgs_out=elastix_multi_dims_2(imgs_in,transform_pars1,transform_pars2,temp,transf_par1,transf_par2,mask,mmask)
...
%%
% this function uses elastix to register a 4D set of images to itself over
% the 4th dimension using 2 tranformations (i.e. a rigid followed by a
% deformable)
...
if nargin==6
    ...
    com=sprintf('elastix -f %s -m %s -out %s -p %s %s -fMask %s -threads 12 > /dev/null',fullfile(temp,'img.nii'),fullfile(temp,'img.nii'),fullfile(temp,'out'),transform_pars1,transform_pars1,fullfile(temp,'mask.nii'));
    system(com);
        ...
        do=sprintf('cp %s %s',fullfile(filepath,'TransformParameters.0.txt'),transf_par1);
        system(do);
        do=sprintf('cp %s %s',fullfile(filepath,'TransformParameters.1.txt'),transf_par2);
        system(do);
        delete(fullfile(filepath,'*'));
        rmdir(fullfile(filepath));
elseif nargin==4
       com=sprintf('elastix -f %s -m %s -out %s -p %s %s -threads 12 > /dev/null',fullfile(temp,'img.nii'),fullfile(temp,'img.nii'),fullfile(temp,'out'),transform_pars1);
    system(com);
        filepath=fullfile(temp,'out');
        if exist(fullfile(filepath,'result.0.nii'),'file')
        else
